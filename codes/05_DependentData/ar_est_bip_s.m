...
x(1:min(10,floor(N/2))) = x_tran(1:min(10,floor(N/2)));
 ...
 if P == 0 % autoregressive order zero
    ...
    a_scale_final(1) = m_scale(x); % AR(0) residual scale equals observation scale

    
 elseif P == 1 % autoregressive order one
   [x_filt, phi_hat, a_scale_final]=bip_ar1_s(x,N,phi_grid,fine_grid,kap2); % AR(1) tau-estimates
    
 elseif P > 1 % autoregressive order p   
   [x_filt, phi_hat(1,1), a_scale_final]=bip_ar1_s(x,N,phi_grid,fine_grid,kap2); % AR(1) tau-estimates
    ...
    for p=2:P
        for mm=1:length(phi_grid)     
            ...
            a_bip_sc(mm)=m_scale(a(p+1:end)); % residual scale for BIP-AR
            a_sc(mm)=m_scale(a2(p+1:end)); % residual scale for AR     
        end
...
function [x_filt, phi_hat, a_scale_final]=bip_ar1_s(x,N,phi_grid,fine_grid,kap2)
...
% grid search for partial autocorrelations   
    ...
    a_bip_sc(mm) = m_scale(a(2:N)); % tau-scale of residuals for BIP-AR
    a_sc(mm) = m_scale(a2(2:N)); % tau-scale of residuals for AR
     end
    ...
    x_filt = zeros(size(x));
        ...
        if temp2<temp
        a_scale_final(2) = m_scale(a(2:N));
        else
        a_scale_final(2) = m_scale(a2(2:N));
        end
